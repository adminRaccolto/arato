...
export async function listarFazendas(id?: string): Promise<Fazenda[]> {
  ...
  if (id) {
    ...
    const isRaccotlo = role === "raccotlo" || role === "raccotlo_gestor" || role === "raccotlo_seletor";

    if (isRaccotlo) {
      // IMPORTANTE: verifica role ANTES de conta_id — raccotlo staff pode ter conta_id
      // de um cliente específico no perfil (ex: Gino está no perfil do Grupo Ogliari),
      // mas ao navegar entre clientes deve usar o cliente ativo do localStorage.
      ...
      if (clienteContaId && !clienteContaId.startsWith("sem_conta_")) {
        ...
        return []; // Sem cliente selecionado — retorna vazio para não vazar dados
      }
    } else if (perfil?.conta_id) {
...
export async function listarInsumosParaConta(contaIdDireto?: string | null, fazendaIdFallback?: string): Promise<Insumo[]> {
  let contaId: string | null = contaIdDireto ?? null;

  // Se não temos contaId direto (ex: raccotlo admin), resolve pela fazenda ativa
  if (!contaId && fazendaIdFallback) {
    const { data: faz } = await supabase.from("fazendas").select("conta_id").eq("id", fazendaIdFallback).maybeSingle();
    contaId = faz?.conta_id ?? null;
  }

  // Último recurso: tenta via perfil do usuário logado (legado)
  if (!contaId) {
    const { data: { user } } = await supabase.auth.getUser();
    if (user) {
