...
**Consulta Sintegra (🔎 ao lado da IE):** na aba "Inscrições Estaduais" do produtor, tanto ao adicionar uma nova IE quanto ao editar o endereço de uma já cadastrada, existe um botão 🔎 que consulta o cadastro de contribuintes direto na SEFAZ (webservice CadConsultaCadastro4) e preenche automaticamente nome, endereço, município e código IBGE a partir da IE digitada — hoje funciona apenas para IEs de Mato Grosso (MT), demais estados retornam aviso de "UF ainda não implementada". Exige que a fazenda já tenha um certificado A1 configurado em Parâmetros → Fiscal (qualquer certificado válido serve, não precisa ser do próprio produtor consultado — a consulta de cadastro é um serviço de busca pública da SEFAZ, não uma emissão de documento).
