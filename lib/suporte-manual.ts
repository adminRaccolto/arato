...
**Correção 24/09/2026 — baixa com desconto por antecipação exigia "nova data de vencimento":** em Contas a Pagar → Registrar pagamento, quando o valor pago somado ao desconto cobre o título inteiro, a baixa agora é total (sem pedir nova data). Antes a tela comparava só o valor pago com o saldo e tratava como pagamento parcial, bloqueando o botão Confirmar baixa. Só é parcial quando pago + desconto ainda deixa saldo.

**Correção 24/09/2026 — CST 41 em toda NF de transferência (regra única):** transferência entre estabelecimentos do mesmo titular (CFOP 5151/6151, 5152/6152, 5409/6409, 5410/6410, 5949/6949) sai com CST 41 (não tributado); venda interna, CST 51 (diferido). O XML já saía certo, mas a impressão local do DANFE e o payload da tela Fiscal → Transferência tinham 51 fixo — agora tudo usa a mesma regra. NF-e de transferência autorizadas ANTES da correção continuam com CST 51 no XML da SEFAZ (o documento autorizado não se altera): se ainda estiver dentro de 24h da autorização, cancele e reemita; depois disso, consulte o contador.

**Novo 24/09/2026 — Expedidor e Recebedor no CT-e:** no formulário, depois do Remetente, use "+ Informar Expedidor" / "+ Informar Recebedor" (opcionais). Escolha um Produtor ou Pessoa cadastrada (preenche nome, CNPJ/CPF, IE e endereço) ou digite. O código IBGE do município é buscado sozinho na transmissão. Se o Tomador do Serviço for Expedidor ou Recebedor, os dados dele são obrigatórios. Aparecem no DACTE e no XML. **Correção junto:** o Tomador enviado à SEFAZ estava fixo em Destinatário independente do escolhido na tela; agora vai o tomador real.

**Regra fiscal — CST do ICMS no CT-e (24/09/2026):** prestação intraestadual (UF de início = UF de fim) sai com **CST 51** (ICMS diferido); interestadual sai com **CST 00** (tributação normal) **ou 20** (com redução de base de cálculo — informe o % de redução). O formulário define sozinho pela origem/destino e só oferece as opções permitidas; a emissão barra qualquer CST fora da regra. O parâmetro "Situação Tributária ICMS Padrão" do emitente vale só como sugestão inicial — a regra por UF prevalece.

**Novo 24/09/2026 — Excluir rascunho de CT-e e MDF-e:** botão "Excluir" (vermelho) ao lado de "Autorizar SEFAZ" nos rascunhos, nas duas telas. Só exclui documento que nunca foi autorizado; autorizado não se apaga (CT-e: Cancelar; MDF-e: Encerrar ou Cancelar). Se o rascunho já tentou transmitir (tem chave de acesso), o aviso pede pra conferir no portal da SEFAZ antes — uma resposta perdida pode significar que ele foi autorizado lá sem o sistema saber, e excluir aqui deixaria um documento (no MDF-e, um manifesto aberto que bloqueia a placa) sem registro. A numeração já usada não é reaproveitada.

**Novo 24/09/2026 — IBS e CBS (Reforma Tributária) no CT-e:** emitente Lucro Presumido/Real é obrigado a destacar IBS/CBS no CT-e desde 05/01/2026 (LC 214/2025, NT 2025.001); Simples Nacional e MEI são dispensados. Configure em **Parâmetros → CT-e**, na transportadora emitente: "Destacar IBS/CBS no CT-e" = Sim, CST (000 = tributação integral, com valores; 410 = imunidade/não incidência, sem valores), cClassTrib de 6 dígitos (000001 = tributação integral padrão) e as alíquotas de IBS (UF e Município) e CBS. 2026 é ano de transição — a referência da lei é CBS 0,9% e IBS 0,1%, mas **confirme com o contador**: os campos vêm vazios de propósito. Com "Sim", o XML leva o grupo IBSCBS e o valor total do documento (serviço + IBS + CBS); se faltar algum dado a emissão bloqueia antes de transmitir. Se o emitente é obrigado e não ativou, o CT-e ainda é emitido, mas aparece um aviso fiscal na autorização. O DACTE impresso ainda não mostra os valores de IBS/CBS (ficam no XML).

**Correção 24/09/2026 — CT-e emitido como CST 51 saía como CST 40 na SEFAZ:** a situação tributária escolhida na tela (00/40/41/51) aparecia certa no DACTE impresso, mas a rota que transmite à SEFAZ não recebia esse campo — o XML caía numa regra antiga (alíquota 0% → CST 40). Corrigido: o CST escolhido agora vai no XML de verdade. CT-e já autorizados antes disso continuam como a SEFAZ registrou (CST 40); se a diferença importar fiscalmente, consulte o contador — o DACTE impresso deles mostra o que foi digitado, não o que a SEFAZ tem.
...
**Correção 24/09/2026 — Transferência entre fazendas com remetente/destinatário trocados:** ao mudar a fazenda de origem ou destino, o CPF/CNPJ e a IE da fazenda anterior ficavam no formulário. Agora são recarregados do cadastro fiscal da fazenda escolhida. Confira IE Emitente e IE Destinatário antes de emitir; NF-e já autorizada com dado errado só se corrige cancelando (dentro de 24h) e emitindo de novo.

**Novo 24/09/2026 — CFOP do CT-e restrito a 5352, 5353 e 6932:** a lista do CT-e tem apenas 5352 (transporte a estabelecimento industrial), 5353 (a estabelecimento comercial) e 6932 (Prestação de Serviço iniciada em outra unidade da federação). O sistema bloqueia a emissão com outro CFOP. CT-e aberto a partir de NF-e vem com 5353; troque se necessário.
...
**Novo 24/09/2026 — Lances de consórcio (dinheiro, embutido, terceiros):** em Financeiro → Consórcios → Registrar Contemplação, use "+ Lance" quantas vezes precisar. Em dinheiro: gera uma conta a pagar na conta bancária escolhida (concilie com o débito do banco). Embutido: não sai caixa — o crédito lançado na conta entra líquido (crédito menos o lance). Recurso de terceiros (FGTS, outro consórcio): só registro, sem conta a pagar. Em "O lance abate o saldo…" escolha: reduzir o valor das parcelas restantes, reduzir o prazo (remove as últimas parcelas) ou não alterar (quando o contrato já refletiu). Confira sempre com o extrato da administradora.
