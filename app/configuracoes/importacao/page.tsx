...
const INSTRUCOES_APOIO = [
  ["INSTRUÇÕES — IMPORTAÇÃO DE APOIO FINANCEIRO"],
  [""],
  ["• Campos com * são obrigatórios. Os demais podem ficar em branco."],
  ["• Não altere os nomes das colunas da linha 1"],
  [""],
  ["COLUNAS PRINCIPAIS"],
  ["• fazenda_nome*: nome EXATO da fazenda conforme cadastrado (ex: Fazenda Matriz)"],
  ["  → Aliases aceitos: Fazenda, Empresa, Filial"],
  ["• descricao*: texto livre — descrição do lançamento (ex: Compra de Defensivos)"],
  ["• categoria*: classificação livre (ex: Defensivos, Corretivos de Solo, Arrendamento)"],
  ["• data_lancamento*: data do lançamento — AAAA-MM-DD (ex: 2025-03-01)"],
  ["• data_vencimento*: data de vencimento — AAAA-MM-DD (ex: 2025-03-30)"],
  ["• valor*: valor sem R$ e sem ponto de milhar (ex: 44200.00)"],
  [""],
  ["FORNECEDOR / PESSOA"],
  ["• pessoa_cpf_cnpj: CPF ou CNPJ do fornecedor — vincula ao cadastro de Pessoas"],
  ["  → Aliases aceitos: CNPJ, CPF, Documento"],
  ["  → Se não informado, o sistema tenta encontrar pelo nome abaixo"],
  ["• pessoa_nome: nome do fornecedor (ex: DIPAGRO LTDA)"],
  ["  → Aliases aceitos: Fornecedor, Credor, Parceiro, Beneficiario, Beneficiário"],
  ["  → O sistema tenta casar o nome com o cadastro de Pessoas (sem acento, sem maiúsculas)"],
  ["  → Se não encontrar no cadastro, grava o texto da planilha para não perder a informação"],
  [""],
  ["PRODUTOR (LCDPR)"],
  ["• produtor_cpf_cnpj: CPF do produtor responsável (para LCDPR e relatórios)"],
  ["  → Aliases aceitos: CPF_Produtor, Produtor_CPF, Emitente_CPF"],
  ["• produtor_nome: nome do produtor (ex: Denis Ogliari)"],
  ["  → Aliases aceitos: Produtor, Emitente, Tomador"],
  ["  → O sistema tenta casar o nome com o cadastro de Produtores (sem acento, sem maiúsculas)"],
  ["  → Se encontrar no cadastro, vincula o ID correto para relatórios e queries"],
  [""],
  ["ANO SAFRA"],
  ["• safra_nome: descrição do ano safra (ex: 2025/2026)"],
  ["  → Aliases aceitos: Safra, AnoSafra, Exercicio, Ano, Safra_Agricola"],
  ["  → O sistema busca o ano safra cadastrado em Cadastros → Safras & Ciclos"],
  ["  → Se não encontrar, grava o texto da planilha para não perder a informação"],
  [""],
  ["OUTROS CAMPOS"],
  ["• moeda: BRL (padrão), USD ou barter"],
  ["• num_parcela / total_parcelas: ex: 1 e 3 = primeira de 3 parcelas"],
  ["• tipo_documento_lcdpr: RECIBO, NF, DUPLICATA, CHEQUE, PIX, TED ou OUTROS"],
  ["• numero_documento: número da NF ou documento (ex: NF 001234, RECIBO 005)"],
  ["• operacao_gerencial: nome exato da operação gerencial cadastrada no sistema"],
  ["  Exemplos: Custeio Soja, Arrendamento Terras, Fretes e Carretos"],
  ["• origem: identificação de origem livre (ex: Talhão 01, Gleba Norte)"],
  ["  → Aliases aceitos: Origem, Centro_Custo, Departamento, Centro Custo"],
  ["• observacao: texto livre de observação"],
  [""],
  ["DICA: IMPORTANDO SUA PLANILHA EXISTENTE"],
  ["  Se a sua planilha já tem os dados mas com nomes de coluna diferentes,"],
  ["  renomeie as colunas para os nomes aceitos acima (ou use os aliases)."],
  ["  O sistema normaliza automaticamente: remove acentos, ignora maiúsculas/minúsculas."],
  ["  Exemplo: coluna 'FORNECEDOR' → reconhecida como pessoa_nome automaticamente."],
];
