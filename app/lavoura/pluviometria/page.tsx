    ...
    <div style={{ padding: "24px 32px", background: "#F4F6FA", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "#888", marginTop: 2 }}>Dados de precipitação por propriedade</div>
        </div>
        {aba === "manual" && (
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 16 }}>
            {[
              ...
              <div key={k.label} style={{ background: "#fff", borderRadius: 12, padding: "14px 18px", border: "0.5px solid #DDE2EE" }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#888", textTransform: "uppercase", marginBottom: 6 }}>{k.label}</div>
                ...
                <thead>
                  <tr style={{ background: "#F8FAFB", borderBottom: "0.5px solid #DDE2EE" }}>
                    {["Data","Hora","Talhão","Chuva (mm)","Duração","Intensidade","Fonte","Obs."].map(h => (
                      <th key={h} style={{ padding: "9px 12px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#555", textTransform: "uppercase", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {leituras.map((l, idx) => {
                    ...
                    return (
                      <tr key={l.id} style={{ borderBottom: "0.5px solid #EEF1F6", background: idx % 2 === 0 ? "#fff" : "#FAFBFC" }}>
                        <td style={{ padding: "9px 12px", fontSize: 13, fontWeight: 600 }}>{fmtData(l.data)}</td>
                        <td style={{ padding: "9px 12px", fontSize: 13, color: "#555" }}>{l.hora?.slice(0,5) ?? "—"}</td>
                        <td style={{ padding: "9px 12px", fontSize: 13, color: "#555" }}>{l.talhao_nome ?? <span style={{ color: "#CCC" }}>Fazenda</span>}</td>
                        <td style={{ padding: "9px 12px", fontSize: 15, fontWeight: 700, color: l.chuva_mm > 30 ? "#E24B4A" : "#1A4870" }}>{fmtN(l.chuva_mm)} mm</td>
                        <td style={{ padding: "9px 12px", fontSize: 13, color: "#555" }}>{l.duracao_min ? `${l.duracao_min} min` : "—"}</td>
                        ...
                        <td style={{ padding: "9px 12px", fontSize: 12, color: "#888" }}>{l.fonte ?? "—"}</td>
                        <td style={{ padding: "9px 12px", fontSize: 12, color: "#555", maxWidth: 180 }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 120, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: 14, width: "100%", maxWidth: 480, boxShadow: "0 12px 40px rgba(0,0,0,0.2)", overflow: "hidden" }}>
            <div style={{ padding: "16px 22px", borderBottom: "0.5px solid #EEF1F6", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button onClick={() => setModalLeit(false)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#888" }}>×</button>
            </div>
            <div style={{ padding: "18px 22px", display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: "#555", display: "block", marginBottom: 4 }}>Data *</label>
                  <input type="date" value={lData} onChange={e => setLData(e.target.value)} style={inp} />
                </div>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: "#555", display: "block", marginBottom: 4 }}>Hora da leitura</label>
                  <input type="time" value={lHora} onChange={e => setLHora(e.target.value)} style={inp} />
                </div>
              </div>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "#555", display: "block", marginBottom: 4 }}>Talhão (opcional)</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "#555", display: "block", marginBottom: 4 }}>Fonte / Equipamento</label>
              ...
              <div style={{ display: "flex", gap: 10, paddingTop: 4 }}>
                <button onClick={() => setModalLeit(false)} style={{ flex: 1, padding: "9px", borderRadius: 8, border: "0.5px solid #D4DCE8", background: "#fff", fontSize: 13, cursor: "pointer", color: "#555" }}>Cancelar</button>
                <button onClick={salvarLeitura} disabled={salvando || !lMm} style={{ flex: 2, padding: "9px", borderRadius: 8, border: "none", background: salvando || !lMm ? "#aaa" : "#1A4870", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
