...
// Mesma tabela de conversão já usada em processarCorrecao/excluirCorrecao
// (lib/db.ts) pra debitar o estoque — precisa ser idêntica aqui pro custo
// bater com a baixa real de estoque.
function converterTonParaUnidadeNativa(ton: number, unidade?: string): number {
  switch (unidade) {
    case "t":  return ton;
    case "g":  return ton * 1_000_000;
    case "sc": return (ton * 1000) / 60;
    default:   return ton * 1000; // kg (padrão) ou não reconhecida
  }
}
...
export default function CorrecaoSoloPage() {
  ...
  useEffect(() => {
    ...
    // lançamentos reais de qualquer outra fazenda do cliente. fazendaFiltro
    // é opcional e explícito (seletor "Todas as fazendas" por padrão).
    ...
    // Conta-wide: a lista de correções mostra lançamentos de qualquer fazenda
    ...
    listarInsumosParaConta(contaId, fazendaId).then(ins => setInsumos(ins.filter(i => i.tipo === "insumo" && i.categoria === "corretivo"))).catch(() => {});
  ...
  const calcItens = itens.filter(it => it.dose_ton_ha && (it.insumo_id || it.produto_nome)).map(it => {
    ...
    // custo_medio é por unidade NATIVA do insumo, não sempre "kg" — o "* 1000"
    // fixo aqui assumia isso e inflava o custo em até 1000x pra corretivo
    // estocado em tonelada (o mais comum pra calcário/gesso). Mesma tabela de
    // conversão já usada em processarCorrecao/excluirCorrecao (lib/db.ts).
    const qtdNativa = converterTonParaUnidadeNativa(qtdTon, ins?.unidade);
    return {
      ...it,
      dose_ton_ha: dose,
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "var(--bg-page)", fontFamily: "system-ui, sans-serif", fontSize: 13 }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <header style={{ background: "var(--bg-card)", borderBottom: "0.5px solid var(--border-table)", padding: "10px 22px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 17, fontWeight: 600, color: "var(--text-1)" }}>Correção de Solo</h1>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex:2000 }}
         >
          <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 26, width: 720, maxWidth: "96vw", maxHeight: "92vh", overflowY: "auto" }}>
            ...
            <div style={{ fontSize: 12, color: "var(--text-2)", marginBottom: 14 }}>Calcário, gesso, micronutrientes e corretivos orgânicos</div>
