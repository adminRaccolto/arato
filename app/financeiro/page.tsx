...
const categoriasCor: Record<string, { bg: string; color: string }> = {
  ...
  "Manutenção":     { bg: "#F1EFE8", color: "var(--text-2)"    },
  "Frete":          { bg: "#FBF3E0", color: "#8B5E14" },
  "Despesas fixas": { bg: "#F1EFE8", color: "var(--text-2)"    },
};
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <header style={{ background: "var(--bg-card)", borderBottom: "0.5px solid var(--border-table)", padding: "10px 22px 0 22px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: 10 }}>
            <div>
              <h1 style={{ margin: 0, fontSize: 17, fontWeight: 600, color: "var(--text-1)" }}>Fluxo de Caixa</h1>
              <p style={{ margin: 0, fontSize: 11, color: "#444" }}>Projeção dia a dia, DFC mensal e análise Previsto × Realizado</p>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 11, color: "var(--text-2)" }}>Cotação: <strong style={{ color: "#7A4300" }}>US$ 1 = R$ {COTACAO_USD.toFixed(2)}</strong></span>
          ...
          {!loading && !erro && (
            <>
              ...
              {lancamentos.length === 0 && (
                <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, padding: 32, textAlign: "center", color: "#444", fontSize: 12 }}>
                ...
                <>
                  ...
                  {aba === "lancamentos" && (
                    <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderTop: "none", borderRadius: "0 0 12px 12px", overflow: "hidden" }}>
                      <div style={{ padding: "12px 16px", borderBottom: "0.5px solid var(--border-row)", display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {([
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        ...
                        <tbody>
                          {lancFiltrados.map((l, li) => {
                            ...
                            const cc = categoriasCor[l.categoria] ?? { bg: "#F1EFE8", color: "var(--text-2)" };
                              ...
                              <tr key={l.id} style={{ borderBottom: li < lancFiltrados.length - 1 ? "0.5px solid var(--border-row)" : "none", background: l.moeda === "barter" ? "#FEF8ED" : "transparent" }}>
                                ...
                                <td style={{ padding: "10px 14px", maxWidth: 220 }}>
                                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                    <span style={{ fontWeight: 600, fontSize: 12, color: "var(--text-1)" }}>{l.descricao}</span>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", borderBottom: "0.5px solid var(--border-row)" }}>
                        ...
                        {subAbaFluxo === "vertical" && (
                          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                            ...
                            <button onClick={diasExpandidos.size > 0 ? recolherTodos : expandirTodos} style={{ fontSize: 11, padding: "5px 12px", borderRadius: 8, border: "0.5px solid var(--border-table)", background: "transparent", color: "#666", cursor: "pointer" }}>
                              ...
                              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
                                ...
                                <tbody>
                                  ...
                                  <tr style={{ background: "#FEF3F3" }}>
                                    ...
                                    {mesesDFC.map(m => (
                                      <td key={m.keyMes} style={{ padding: "7px 10px", textAlign: "right", fontSize: 11, fontWeight: 600, color: totSaidas(m.keyMes) > 0 ? "#E24B4A" : "var(--text-3)" }}>
                                  ...
                                  <tr style={{ background: "#F8F9FB", borderTop: "0.5px solid var(--border-table)" }}>
                                    <td style={{ padding: "7px 14px", fontSize: 11, fontWeight: 600, color: "var(--text-2)", position: "sticky", left: 0, background: "#F8F9FB", zIndex: 1 }}>= Diferença</td>
                                    {mesesDFC.map(m => {
                                      const d = diferenca(m.keyMes);
                                      return (
                                        <td key={m.keyMes} style={{ padding: "7px 10px", textAlign: "right", fontSize: 11, fontWeight: 600, color: d > 0 ? "#16A34A" : d < 0 ? "#E24B4A" : "var(--text-3)" }}>
                                          {d !== 0 ? fmtBRL(d) : "—"}
                                        </td>
                                      );
                                    })}
                                    <td style={{ padding: "7px 10px", textAlign: "right", fontSize: 11, fontWeight: 700, color: "var(--text-2)" }}>
                          ...
                          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
                            <thead>
                              <tr style={{ background: "var(--bg-page)" }}>
                                ...
                                {saldosMes.map(m => (
                                  <th key={m.keyMes} style={{ padding: "10px 12px", textAlign: "right", fontSize: 11, fontWeight: 600, color: m.passado ? "#444" : "var(--text-1)", borderBottom: "0.5px solid var(--border-table)", whiteSpace: "nowrap" }}>
                                    {m.label}
                            ...
                            <tbody>
                              ...
                              {/* Saídas */}
                              <tr style={{ background: "#FEF3F3" }}>
                                <td style={{ padding: "7px 16px", fontSize: 11, fontWeight: 600, color: "#791F1F", borderBottom: "0.5px solid var(--border-table)" }}>▼ Saídas</td>
                                {saldosMes.map(m => (
                                  <td key={m.keyMes} style={{ padding: "7px 12px", textAlign: "right", fontSize: 11, fontWeight: 600, color: "#E24B4A", borderBottom: "0.5px solid var(--border-table)" }}>
                            ...
                            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
                                ...
                                <tr style={{ background: "var(--bg-page)" }}>
                                  ...
                                  {prevReal.map(m => (
                                    <th key={m.keyMes} style={{ padding: "9px 10px", textAlign: "right", fontSize: 11, fontWeight: 600, color: m.isFuturo ? "var(--text-3)" : "var(--text-1)", borderBottom: "0.5px solid var(--border-table)", whiteSpace: "nowrap" }}>
                                      {m.label}
                                      {m.isFuturo && <div style={{ fontSize: 9, color: "var(--text-muted)", fontWeight: 400 }}>previsto</div>}
                                    </th>
                                  ))}
                                </tr>
                              </thead>
                              <tbody>
                                ...
                                {(["entPrev", "entReal"] as const).map(k => (
                                  <tr key={k} style={{ borderBottom: k === "entReal" ? "2px solid var(--border-table)" : "0.5px solid #f0f3f8" }}>
                                    <td style={{ padding: "7px 16px", fontSize: 11, color: "var(--text-1)", position: "sticky", left: 0, background: "var(--bg-card)" }}>
                                ...
                                {/* Saídas */}
                                <tr style={{ background: "#FEF3F3" }}>
                                  <td colSpan={prevReal.length + 1} style={{ padding: "5px 16px", fontSize: 10, fontWeight: 700, color: "#791F1F", letterSpacing: "0.05em", borderBottom: "0.5px solid var(--border-table)" }}>
                                    ▼ SAÍDAS (CP)
                                  </td>
                                </tr>
                                {(["saiPrev", "saiReal"] as const).map(k => (
                                  <tr key={k} style={{ borderBottom: k === "saiReal" ? "2px solid var(--border-table)" : "0.5px solid #f0f3f8" }}>
                                    <td style={{ padding: "7px 16px", fontSize: 11, color: "var(--text-1)", position: "sticky", left: 0, background: "var(--bg-card)" }}>
                                    ...
                                    {prevReal.map(m => {
                                      const val = m[k] as number;
                                      return (
                                        <td key={m.keyMes} style={{ padding: "7px 10px", textAlign: "right", fontSize: 11, color: val > 0 ? "#E24B4A" : "var(--text-3)", fontWeight: k === "saiReal" ? 600 : 400 }}>
                                          {val > 0 ? fmtBRL(val) : "—"}
                                ...
                                {(["saldoPrev", "saldoReal"] as const).map(k => (
                                  <tr key={k} style={{ borderBottom: "0.5px solid #f0f3f8", background: k === "saldoReal" ? "#F9F9F6" : "var(--bg-card)" }}>
                                    <td style={{ padding: "8px 16px", fontSize: 12, fontWeight: 600, color: "var(--text-1)", position: "sticky", left: 0, background: k === "saldoReal" ? "#F9F9F6" : "var(--bg-card)" }}>
                            ...
                            <div style={{ display: "flex", alignItems: "stretch", background: "var(--bg-card)", borderBottom: "0.5px solid var(--border-table)" }}>
                              <div style={{ flex: 1, display: "grid", gridTemplateColumns: "repeat(3, 1fr)" }}>
                                {[
                                  ...
                                  <div key={i} style={{ padding: "12px 16px", borderRight: "0.5px solid var(--border-row)" }}>
                                    <div style={{ fontSize: 11, color: "var(--text-2)", marginBottom: 3 }}>{s.label}</div>
                                    <div style={{ fontSize: 16, fontWeight: 600, color: s.cor }}>{fmtBRL(Math.abs(s.valor))}</div>
                                  </div>
                                ))}
                              </div>
                              {/* Botão único de nova simulação */}
                              <div style={{ display: "flex", alignItems: "center", padding: "0 16px", borderLeft: "0.5px solid var(--border-row)", gap: 8 }}>
                                ...
                                return (
                                  <div key={dia} style={{ borderBottom: "0.5px solid #e0e0dc" }}>
                                    ...
                                    >
                                      ...
                                      {/* Saldo do dia */}
                                      <div style={{ textAlign: "right", fontSize: 12, fontWeight: 600, color: !temEventos ? "#666" : saldoDia >= 0 ? "var(--text-1)" : "#E24B4A" }}>
                                        ...
                                        {linhas.map((item, idx) => (
                                          <div key={item.key} style={{ display: "grid", gridTemplateColumns: GRID, alignItems: "center", padding: "5px 14px", background: idx % 2 === 0 ? "#EEF1F8" : "#EEECEA", borderBottom: idx < linhas.length - 1 ? "0.5px solid #e8e8e5" : "none" }}>
                                            ...
                                            <div style={{ display: "flex", alignItems: "center", gap: 6, paddingLeft: 24 }}>
                                              ...
                                              <span style={{ fontSize: 11, color: "var(--text-1)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={item.label}>{item.label}</span>
                                              {item.sub && <span style={{ fontSize: 10, color: "var(--text-3)", whiteSpace: "nowrap", flexShrink: 0 }}>{item.sub}</span>}
                                            </div>
                                            {/* Status */}
                                            ...
                                            <div style={{ textAlign: "right", paddingRight: 6 }}>
                                              ...
                                              {item.cpVal !== undefined && item.subMoeda && <div style={{ fontSize: 9, color: "var(--text-3)" }}>{item.subMoeda}</div>}
                                            </div>
                                            {/* Sim */}
                                            ...
                                            <div style={{ textAlign: "right" }}>
                                              {idx === linhas.length - 1 && (
                                                <span style={{ fontSize: 11, fontWeight: 600, color: saldoDia >= 0 ? "var(--text-1)" : "#E24B4A" }}>{fmtBRL(saldoDia)}</span>
                                              )}
                                            </div>
      ...
      {modalBaixa && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex:2000 }}
          ...
          <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 26, width: 440, maxWidth: "92vw" }}>
            ...
            <div style={{ fontSize: 12, color: "var(--text-2)", marginBottom: 20 }}>{modalBaixa.descricao}</div>
            ...
            {modalBaixa.moeda === "barter" ? (
              <div style={{ display: "grid", gap: 14 }}>
                <div style={{ background: "#FBF3E0", border: "0.5px solid #8B5E1430", borderRadius: 8, padding: "10px 12px", fontSize: 12, color: "#8B5E14" }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--text-2)", marginTop: 3 }}>Equivalente gerencial: {fmtBRL(modalBaixa.valor)} · sem movimentação bancária</div>
                </div>
                <div>
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex:2000 }}
            ...
            <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 26, width: 560, maxWidth: "94vw", maxHeight: "92vh", overflowY: "auto" }}>
              ...
              {/* ── Parcelamento ── */}
              <div style={{ marginTop: 18, borderTop: "0.5px solid var(--border-row)", paddingTop: 16 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13, fontWeight: 600, color: "var(--text-1)", marginBottom: novoLanc.parcelar ? 12 : 0 }}>
              ...
              {/* ── Adicionais ── */}
              <div style={{ marginTop: 18, borderTop: "0.5px solid var(--border-row)", paddingTop: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text-2)", marginBottom: 12 }}>Adicionais — LCDPR, encargos e vínculos</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  <div>
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 20 }}>
                <button onClick={() => setModalNovo(false)} style={{ padding: "8px 18px", border: "0.5px solid var(--border-table)", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 13 }}>Cancelar</button>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex:2000 }}
          ...
          <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 26, width: 520, maxWidth: "92vw", maxHeight: "85vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 16, color: "var(--text-1)" }}>Previsão de Gastos</div>
                <div style={{ fontSize: 11, color: "var(--text-2)", marginTop: 2 }}>Aparecem em verde no fluxo · podem ser convertidas em CP</div>
              </div>
              <div style={{ display: "flex", gap: 4 }}>
                ...
                {previsoes.length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#444", marginTop: 4 }}>
                      Total previsto: <strong style={{ color: "var(--text-1)" }}>{fmtBRL(previsoes.filter(p => p.tipo === "pagar").reduce((a, p) => a + p.valor, 0))}</strong> em saídas ·{" "}
                      <strong style={{ color: "var(--text-1)" }}>{fmtBRL(previsoes.filter(p => p.tipo === "receber").reduce((a, p) => a + p.valor, 0))}</strong> em entradas
                    </div>
                  </div>
                )}
              </div>
            ...
            <div style={{ marginTop: 16, display: "flex", justifyContent: "flex-end" }}>
              <button onClick={() => setModalPrevisao(false)} style={{ padding: "8px 18px", border: "0.5px solid var(--border-table)", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 13 }}>Fechar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalSimDia && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex:2000 }}
          ...
          <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 26, width: 400, maxWidth: "92vw" }}>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 18 }}>
              <button onClick={() => setModalSimDia(null)} style={{ padding: "8px 18px", border: "0.5px solid var(--border-table)", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 13 }}>Cancelar</button>
