...
export default function CampoMonitoramentoPage() {
  ...
  if (etapa === "gps") return (
    <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ fontSize: 16, fontWeight: 700, color: "var(--text-1)" }}>📍 Capturar Localização</div>
      <div style={{ fontSize: 13, color: "var(--text-2)" }}>A localização GPS georreferencia o ponto de incidência no talhão.</div>

      <div style={{ background: gpsLat ? "#F0FDF4" : "var(--bg-page)", border: `0.5px solid ${gpsLat ? "#86EFAC" : "var(--border)"}`, borderRadius: 14, padding: 20, textAlign: "center" }}>
        {gpsBusy ? (
          <div>
            <div style={{ fontSize: 40, marginBottom: 12 }}>🔄</div>
            <div style={{ fontSize: 14, color: "var(--text-2)" }}>Obtendo localização...</div>
            <div style={{ fontSize: 12, color: "var(--text-3)", marginTop: 4 }}>Aguarde até 20 segundos</div>
          </div>
        ) : gpsLat ? (
          <div>
            <div style={{ fontSize: 40, marginBottom: 12 }}>✅</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#166534" }}>Localização capturada</div>
            <div style={{ fontSize: 13, color: "var(--text-2)", marginTop: 6, fontFamily: "monospace" }}>
              {gpsLat.toFixed(7)}<br />{gpsLng?.toFixed(7)}
            </div>
            <div style={{ fontSize: 12, color: "var(--text-3)", marginTop: 4 }}>{gpsMsg}</div>
          </div>
        ) : (
          <div>
            <div style={{ fontSize: 40, marginBottom: 12 }}>📡</div>
            <div style={{ fontSize: 14, color: "var(--text-2)" }}>GPS não capturado</div>
  ...
  if (etapa === "foto") return (
    <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ fontSize: 16, fontWeight: 700, color: "var(--text-1)" }}>📷 Foto da Ocorrência</div>
  ...
  return (
    <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div>
        <div style={{ fontSize: 16, fontWeight: 700, color: "var(--text-1)" }}>🐛 Monitoramento de Campo</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <div>
          <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)", display: "block", marginBottom: 6 }}>Ciclo</label>
      ...
      {/* Tipo */}
      <div>
        <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)", display: "block", marginBottom: 6 }}>Tipo *</label>
      ...
      {/* Ocorrência */}
      <div>
        <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)", display: "block", marginBottom: 6 }}>Ocorrência *</label>
      ...
      {/* Nível */}
      <div>
        <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)", display: "block", marginBottom: 6 }}>Nível de Infestação *</label>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
          {NIVEL.map(n => (
            <button key={n.n} type="button" onClick={() => setFNivel(n.n)}
              style={{ padding: "12px", borderRadius: 12, border: `2px solid ${fNivel === n.n ? n.cor : "var(--border)"}`, background: fNivel === n.n ? n.bg : "var(--bg-card)", cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <div>
          <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)", display: "block", marginBottom: 6 }}>% Plantas Afetadas</label>
          <InputNumerico min="0" max="100" placeholder="Ex: 15" value={fPct} onChange={v => setFPct(v)} style={inp} />
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)", display: "block", marginBottom: 6 }}>Estágio</label>
      ...
      {/* Ação recomendada */}
      <div>
        <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)", display: "block", marginBottom: 6 }}>Ação Recomendada</label>
      ...
      {recs.length > 0 && (
        <div>
          <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)", display: "block", marginBottom: 6 }}>Vincular Recomendação</label>
