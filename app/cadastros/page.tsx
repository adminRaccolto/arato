...
import {
  ...
  listarInsumosParaConta, criarInsumo, atualizarInsumo, excluirInsumo,
...
const TAB_GROUPS: TabGroup[] = [
  ...
  { group: "Cadastros Técnicos", tabs: [
    ...
    { key: "combustivel",             label: "Combustíveis & Lubrificantes" },
...
function CadastrosInner() {
  ...
  const [subAbaComb, setSubAbaComb]   = useState<"bombas"|"combustiveis"|"lubrificantes">("bombas");
  const [filtroComb, setFiltroComb]   = useState("todos");
  const [buscaComb, setBuscaComb]     = useState("");
  const [filtroLubri, setFiltroLubri] = useState("todos");
  const [buscaLubri, setBuscaLubri]   = useState("");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "var(--bg-page)", fontFamily: "system-ui, sans-serif", fontSize: 13 }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <header style={{ background: "var(--bg-card)", borderBottom: "0.5px solid var(--border-table)", padding: "10px 22px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16 }}>
          <div>
        ...
        <div style={{ padding: "20px 22px", flex: 1, overflowY: "auto" }}>
            ...
            <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  ...
                  {produtores.map((p, i) => (
                    <tr key={p.id} style={{ borderBottom: i < produtores.length - 1 ? "0.5px solid var(--border-row)" : "none" }}>
                    ...
                    <div key={f.id} style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                      ...
                      {exp && (
                        <div style={{ borderTop: "0.5px solid var(--border-row)" }}>
                          ...
                          {/* Matrículas */}
                          <div style={{ padding: "10px 16px", borderBottom: "0.5px solid var(--border-row)" }}>
                              ...
                              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                                ...
                                <tbody>
                                  {mats.map((m, mi) => {
                                    ...
                                    return (
                                      <tr key={m.id} style={{ borderBottom: mi < mats.length - 1 ? "0.5px solid var(--border-row)" : "none" }}>
                          ...
                          <div style={{ padding: "10px 16px" }}>
                            ...
                            {tals.length === 0 ? <div style={{ fontSize: 12, color: "#444", padding: "6px 0" }}>Nenhum talhão cadastrado</div> : (
                              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                                ...
                                <tbody>
                                  {tals.map((t, ti) => (
                                    <tr key={t.id} style={{ borderBottom: ti < tals.length - 1 ? "0.5px solid var(--border-row)" : "none" }}>
              ...
              <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900 }}>
                  ...
                  <tbody>
                    ...
                    {pessoasFilt.map((p, i) => (
                      <tr key={p.id} style={{ borderBottom: i < pessoasFilt.length - 1 ? "0.5px solid var(--border-row)" : "none" }}>
                        <td style={{ padding: "10px 14px", color: "var(--text-1)", fontWeight: 600 }}>{p.nome}</td>
          ...
          {aba === "maquinas" && (() => {
            ...
            const thS: React.CSSProperties = { padding: "8px 14px", textAlign: "center" as const, fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)", whiteSpace: "nowrap" as const };
            return (
            <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
          ...
          {aba === "combustivel" && (() => {
            const combBase  = insumos.filter(i => i.categoria === "combustivel");
            const lubriBase = insumos.filter(i => i.categoria === "lubrificante");

            const combFiltr  = combBase.filter(i => {
              const matchSub  = filtroComb  === "todos" || i.subgrupo === filtroComb;
              const matchBusca = !buscaComb  || i.nome.toLowerCase().includes(buscaComb.toLowerCase());
              return matchSub && matchBusca;
            });
            const lubriFiltr = lubriBase.filter(i => {
              const matchSub  = filtroLubri === "todos" || i.subgrupo === filtroLubri;
              const matchBusca = !buscaLubri || i.nome.toLowerCase().includes(buscaLubri.toLowerCase());
              return matchSub && matchBusca;
            });

            const abrirModalComb = (cat: "combustivel"|"lubrificante", ins?: Insumo) => {
              setEditIns(ins ?? null);
              const defaultUnit = cat === "lubrificante" ? "L" : "L";
              setFIns(ins ? {
                nome: ins.nome, categoria: ins.categoria, subgrupo: ins.subgrupo ?? "",
                cultura_id: "", ncm: "", unidade: ins.unidade, fabricante: ins.fabricante ?? "",
            ...
            const SUBG_COMB  = ["Diesel S10","Diesel S500","Gasolina Comum","Gasolina Aditivada","Etanol","ARLA 32"];
            const SUBG_LUBRI = ["Óleo Motor","Óleo Hidráulico","Óleo Câmbio","Óleo Diferencial","Graxa","Fluido de Freio","Óleo 2T"];

            const SUB_ABAS_COMB = [
              { key: "bombas",       label: "Bombas & Tanques" },
              { key: "combustiveis", label: `Catálogo Combustíveis (${combBase.length})`  },
              { key: "lubrificantes",label: `Lubrificantes (${lubriBase.length})` },
            ] as const;
            ...
            return (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                {/* ─── Catálogo Combustíveis ─── */}
                {subAbaComb === "combustiveis" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, padding: "12px 16px", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                      <input style={{ ...inp, width: 200 }} placeholder="Buscar…" value={buscaComb} onChange={e => setBuscaComb(e.target.value)} />
                      <div style={{ display: "flex", gap: 6, flex: 1, flexWrap: "wrap" }}>
                        <button onClick={() => setFiltroComb("todos")} style={{ padding: "5px 12px", borderRadius: 20, border: "0.5px solid", borderColor: filtroComb === "todos" ? "#0C447C" : "var(--border-table)", background: filtroComb === "todos" ? "#E6F1FB" : "transparent", color: filtroComb === "todos" ? "#0C447C" : "#666", fontSize: 12, cursor: "pointer", fontWeight: filtroComb === "todos" ? 600 : 400 }}>Todos ({combBase.length})</button>
                        {SUBG_COMB.filter(s => combBase.some(i => i.subgrupo === s)).map(s => (
                          <button key={s} onClick={() => setFiltroComb(s)} style={{ padding: "5px 12px", borderRadius: 20, border: "0.5px solid", borderColor: filtroComb === s ? "#0C447C" : "var(--border-table)", background: filtroComb === s ? "#E6F1FB" : "transparent", color: filtroComb === s ? "#0C447C" : "#666", fontSize: 12, cursor: "pointer", fontWeight: filtroComb === s ? 600 : 400 }}>
                            {s} ({combBase.filter(i => i.subgrupo === s).length})
                          </button>
                        ))}
                      </div>
                      <button style={btnV} onClick={() => abrirModalComb("combustivel")}>+ Novo Combustível</button>
                    </div>
                    <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                      {combFiltr.length === 0 ? (
                        <div style={{ padding: 40, textAlign: "center", color: "#444", fontSize: 13 }}>Nenhum combustível cadastrado. Ex: Diesel S10, Gasolina, ARLA 32.</div>
                      ) : (
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                          <TH cols={["Nome", "Tipo", "Unid.", "Estoque (L)", "Mín.", "Valor (R$/L)", "Total", ""]} />
                          <tbody>
                            {combFiltr.map((ins, i) => {
                              const abaixo = ins.estoque < ins.estoque_minimo;
                              return (
                                <tr key={ins.id} style={{ borderBottom: i < combFiltr.length - 1 ? "0.5px solid var(--border-row)" : "none", background: abaixo ? "#FFFAF5" : "transparent" }}>
                                  <td style={{ padding: "10px 14px", fontWeight: 600 }}>{ins.nome}</td>
                                  <td style={{ padding: "10px 14px", textAlign: "center" }}>{ins.subgrupo ? badge(ins.subgrupo, "#E6F1FB","#0C447C") : "—"}</td>
                                  ...
                                  <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 600, color: abaixo ? "#E24B4A" : "var(--text-1)" }}>
                                    {ins.estoque.toLocaleString("pt-BR",{maximumFractionDigits:1})}
                                    {abaixo && <span style={{ marginLeft:5,fontSize:9,background:"#FCEBEB",color:"#791F1F",padding:"1px 5px",borderRadius:6 }}>⚠ baixo</span>}
                                  </td>
                                  <td style={{ padding: "10px 14px", textAlign: "right", fontSize: 12 }}>{ins.estoque_minimo.toLocaleString("pt-BR",{maximumFractionDigits:1})}</td>
                                  ...
                                  <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, fontSize: 12 }}>{(ins.estoque*ins.valor_unitario).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}</td>
                                  <td style={{ padding: "10px 14px", textAlign: "right" }}>
                                    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                                      <button style={btnE} onClick={() => abrirModalComb("combustivel", ins)}>Editar</button>
                ...
                {/* ─── Lubrificantes ─── */}
                {subAbaComb === "lubrificantes" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, padding: "12px 16px", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                      <input style={{ ...inp, width: 200 }} placeholder="Buscar…" value={buscaLubri} onChange={e => setBuscaLubri(e.target.value)} />
                      <div style={{ display: "flex", gap: 6, flex: 1, flexWrap: "wrap" }}>
                        <button onClick={() => setFiltroLubri("todos")} style={{ padding: "5px 12px", borderRadius: 20, border: "0.5px solid", borderColor: filtroLubri === "todos" ? "#7A4300" : "var(--border-table)", background: filtroLubri === "todos" ? "#FEF3E2" : "transparent", color: filtroLubri === "todos" ? "#7A4300" : "#666", fontSize: 12, cursor: "pointer", fontWeight: filtroLubri === "todos" ? 600 : 400 }}>Todos ({lubriBase.length})</button>
                        {SUBG_LUBRI.filter(s => lubriBase.some(i => i.subgrupo === s)).map(s => (
                          <button key={s} onClick={() => setFiltroLubri(s)} style={{ padding: "5px 12px", borderRadius: 20, border: "0.5px solid", borderColor: filtroLubri === s ? "#7A4300" : "var(--border-table)", background: filtroLubri === s ? "#FEF3E2" : "transparent", color: filtroLubri === s ? "#7A4300" : "#666", fontSize: 12, cursor: "pointer", fontWeight: filtroLubri === s ? 600 : 400 }}>
                            {s} ({lubriBase.filter(i => i.subgrupo === s).length})
                          </button>
                        ))}
                      </div>
                      <button style={btnV} onClick={() => abrirModalComb("lubrificante")}>+ Novo Lubrificante</button>
                    </div>
                    <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                      {lubriFiltr.length === 0 ? (
                        <div style={{ padding: 40, textAlign: "center", color: "#444", fontSize: 13 }}>Nenhum lubrificante cadastrado. Ex: Óleo Motor, Óleo Hidráulico, Graxa.</div>
                      ) : (
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                          <TH cols={["Nome / Fabricante", "Tipo", "Unid.", "Estoque", "Mín.", "Valor Unit.", "Total", ""]} />
                          <tbody>
                            {lubriFiltr.map((ins, i) => {
                              const abaixo = ins.estoque < ins.estoque_minimo;
                              return (
                                <tr key={ins.id} style={{ borderBottom: i < lubriFiltr.length - 1 ? "0.5px solid var(--border-row)" : "none", background: abaixo ? "#FFFAF5" : "transparent" }}>
                                  <td style={{ padding: "10px 14px" }}>
                                    <div style={{ fontWeight: 600 }}>{ins.nome}</div>
                                    {ins.fabricante && <div style={{ fontSize: 11, color: "var(--text-2)" }}>{ins.fabricante}</div>}
                                  </td>
                                  <td style={{ padding: "10px 14px", textAlign: "center" }}>{ins.subgrupo ? badge(ins.subgrupo, "#FEF3E2","#7A4300") : "—"}</td>
                                  <td style={{ padding: "10px 14px", textAlign: "center", fontSize: 12 }}>{ins.unidade}</td>
                                  <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 600, color: abaixo ? "#E24B4A" : "var(--text-1)" }}>
                                  ...
                                  <td style={{ padding: "10px 14px", textAlign: "right" }}>
                                    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                                      <button style={btnE} onClick={() => abrirModalComb("lubrificante", ins)}>Editar</button>
                                      <button style={btnX} onClick={() => { if (confirm(`Excluir "${ins.nome}"?`)) excluirInsumo(ins.id).then(() => setInsumos(x => x.filter(r => r.id !== ins.id))); }}>✕</button>
                                    </div>
                                  </td>
                                </tr>
                ...
                {/* Modal Combustível / Lubrificante */}
                {modalIns && (fIns.categoria === "combustivel" || fIns.categoria === "lubrificante") && (() => {
                  const isLubri = fIns.categoria === "lubrificante";
                  const sugestoes = isLubri ? SUBG_LUBRI : SUBG_COMB;
                  return (
                    <Modal titulo={editIns ? `Editar: ${editIns.nome}` : isLubri ? "Novo Lubrificante" : "Novo Combustível"} onClose={() => setModalIns(false)} width={680}>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                        <div>
                          <label style={lbl}>Nome *</label>
                          <input style={inp} placeholder={isLubri ? "Ex: Shell Helix HX7 15W40" : "Ex: Diesel S10"} value={fIns.nome} onChange={e => setFIns(p => ({ ...p, nome: e.target.value }))} />
                        </div>
                        <div>
                          <label style={lbl}>{isLubri ? "Tipo" : "Tipo de Combustível"}</label>
                          <input list={`subg-comb-list-${fIns.categoria}`} style={inp} placeholder={`Ex: ${sugestoes[0]}`} value={fIns.subgrupo} onChange={e => setFIns(p => ({ ...p, subgrupo: e.target.value }))} />
                          <datalist id={`subg-comb-list-${fIns.categoria}`}>{sugestoes.map(s => <option key={s} value={s} />)}</datalist>
                        </div>
                        {isLubri && (
                          <div>
                            <label style={lbl}>Fabricante / Marca</label>
                            <input style={inp} placeholder="Ex: Shell, Mobil, Castrol" value={fIns.fabricante} onChange={e => setFIns(p => ({ ...p, fabricante: e.target.value }))} />
                          </div>
                        )}
                        <div>
                          ...
                          <select style={inp} value={fIns.unidade} onChange={e => setFIns(p => ({ ...p, unidade: e.target.value as Insumo["unidade"] }))}>
                            ...
                            <option value="kg">kg (graxas em kg)</option>
                            <option value="un">un (galões/latas)</option>
                          </select>
                        </div>
                        <div>
                          <label style={lbl}>Estoque atual ({fIns.unidade})</label>
                          {editIns ? (
                            <div style={{ padding: "8px 10px", background: "var(--bg-page)", borderRadius: 7, border: "0.5px solid var(--border)", fontSize: 13, color: "var(--text-3)" }}>
                              {fIns.estoque} {fIns.unidade} — altere via Ajuste de Estoque
                            </div>
                          ) : (
                            <InputMonetario style={inp} value={fIns.estoque} onChange={v => setFIns(p => ({ ...p, estoque: String(v) }))} />
                          )}
                        </div>
                        <div>
                          <label style={lbl}>Estoque mínimo (alerta)</label>
                          <InputMonetario style={inp} value={fIns.estoque_minimo} onChange={v => setFIns(p => ({ ...p, estoque_minimo: String(v) }))} />
                        </div>
                        <div>
          ...
          {aba === "grupos_insumo" && (
            <div style={{ display: "flex", gap: 16 }}>
              {/* Grupos */}
              <div style={{ flex: 1, background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                <div style={{ padding: "14px 18px", borderBottom: "0.5px solid var(--border-row)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
              ...
              <div style={{ flex: 1, background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                ...
                <div style={{ overflowY: "auto", maxHeight: 520 }}>
                  ...
                  {gruposInsumo.map(g => {
                    ...
                    return (
                      <div key={g.id}>
                        <div style={{ padding: "7px 14px", background: "var(--bg-page)", borderBottom: "0.5px solid var(--border-row)", display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        {subs.map((s, i) => (
                          <div key={s.id} style={{ display: "flex", alignItems: "center", padding: "8px 14px 8px 32px", borderBottom: i < subs.length - 1 ? "0.5px solid #F0F3F8" : "0.5px solid var(--border-row)" }}>
          ...
          {(aba as string) === "auxiliares" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {/* Sub-abas */}
              <div style={{ display: "flex", gap: 0, background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 8, overflow: "hidden", width: "fit-content" }}>
                {([
                  { key: "grupos_insumo", label: "Grupos de Insumos" },
                ...
                <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                      ...
                      {tiposPessoa.map((t, i) => (
                        <tr key={t.id} style={{ borderBottom: i < tiposPessoa.length - 1 ? "0.5px solid var(--border-row)" : "none" }}>
              ...
              {subAbaAux === "centros_custo" && (
                <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                  <div style={{ padding: "14px 18px", borderBottom: "0.5px solid var(--border-row)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button style={btnV} onClick={() => { setEditCC(null); setFCC({ fazenda_id: fazIdEff || "", codigo: "", nome: "", tipo: "despesa", parent_id: "", manutencao_maquinas: false }); setModalCC(true); }}>+ Novo</button>
                  </div>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                      ...
                      {categoriasLanc.map((c, i) => {
                        ...
                        return (
                          <tr key={c.id} style={{ borderBottom: i < categoriasLanc.length - 1 ? "0.5px solid var(--border-row)" : "none" }}>
                            <td style={{ padding: "10px 14px", color: "var(--text-1)", fontWeight: 600 }}>{c.nome}</td>
          ...
          {aba === "centros_custo" && (
            <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {(() => {
                    const lista = centrosCusto;
                    if (lista.length === 0) return <tr><td colSpan={6} style={{ padding: 32, textAlign: "center", color: "#444" }}>Nenhum centro de custo cadastrado</td></tr>;
                  ...
                  <button
                    onClick={() => {
                      ...
                      const linhas = sorted.map(o => {
                        ...
                        return `<tr style="background:${nivel===0?"#F0F4FA":"#fff"};border-bottom:0.5px solid var(--border)">
                    ...
                    style={{ fontSize: 13, padding: "8px 14px", border: "0.5px solid var(--border)", borderRadius: 8, background: "var(--bg-page)", color: "var(--text-2)", cursor: "pointer", fontWeight: 600 }}
                  >🖨 Imprimir</button>
                  <button
                    onClick={async () => {
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  ...
                  {[...opGers].sort((a, b) => a.classificacao.localeCompare(b.classificacao, "pt-BR", { numeric: false })).map((o, i, arr) => {
                    ...
                    return (
                      <tr key={o.id} style={{ borderBottom: i < arr.length - 1 ? "0.5px solid var(--border-row)" : "none", opacity: o.inativo ? 0.5 : 1, background: nivel === 0 ? "var(--bg-card)" : "var(--bg-card)" }}>
                        ...
                        <td style={{ padding: "9px 14px" }}>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                            {telas.map(t => <span key={t} style={{ fontSize: 10, background: "var(--bg-page)", color: "var(--text-2)", padding: "2px 6px", borderRadius: 4 }}>{t}</span>)}
                          </div>
                        </td>
                ...
                <div style={{ padding: "14px 18px", borderBottom: "0.5px solid var(--border-row)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button
                        ...
                        style={{ fontSize: 12, padding: "7px 14px", border: "0.5px solid var(--border)", borderRadius: 8, background: "var(--bg-page)", color: "var(--text-2)", cursor: "pointer" }}
                      >↺ Atualizar</button>
                    </div>
                  </div>
                  {/* Filtros */}
                  <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                    <input
                      ...
                      style={{ flex: "1 1 220px", minWidth: 180, padding: "7px 10px", border: "0.5px solid var(--border-table)", borderRadius: 8, fontSize: 13 }}
                    />
                    <input
                      value={hfFiltroCfop} onChange={e => setHfFiltroCfop(e.target.value)}
                      placeholder="Filtra por CFOP ex: 5101"
                      style={{ width: 130, padding: "7px 10px", border: "0.5px solid var(--border-table)", borderRadius: 8, fontSize: 13 }}
                    />
                    <select value={hfFiltroTipo} onChange={e => setHfFiltroTipo(e.target.value as ""| "receita"|"despesa")}
                      style={{ padding: "7px 10px", border: "0.5px solid var(--border-table)", borderRadius: 8, fontSize: 13 }}>
                      <option value="">Receita + Despesa</option>
                      <option value="receita">Receitas</option>
                      <option value="despesa">Despesas</option>
                    </select>
                    <select value={hfFiltroNf} onChange={e => setHfFiltroNf(e.target.value)}
                      style={{ padding: "7px 10px", border: "0.5px solid var(--border-table)", borderRadius: 8, fontSize: 13, maxWidth: 220 }}>
          ...
          {aba === "insumos" && (() => {
            ...
            // Legado: micronutriente/biologico/inoculante mapeados para o grupo pai
            const LEGACY_CAT_MAP: Record<string, Insumo["categoria"]> = {
              micronutriente: "fertilizante",
              biologico:      "defensivo",
              inoculante:     "semente",
            };
            const catMap: Record<string, { label: string; bg: string; cl: string }> = {
            ...
            const insFiltr = insBase.filter(i => {
              const catEfetiva = LEGACY_CAT_MAP[i.categoria] ?? i.categoria;
              const matchCat   = filtroIns === "todos" || catEfetiva === filtroIns || i.categoria === filtroIns;
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, padding: "12px 16px", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flex: 1, flexWrap: "wrap" }}>
                    ...
                    {CATS.map(c => {
                      // Conta itens do grupo + legados mapeados para ele
                      const qtd = insBase.filter(i => {
                        const ef = LEGACY_CAT_MAP[i.categoria] ?? i.categoria;
                        return ef === c.key;
                      }).length;
                ...
                <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                  {insFiltr.length === 0 ? (
                    <div style={{ padding: 40, textAlign: "center", color: "#444", fontSize: 13 }}>
                      {buscaIns ? `Nenhum insumo encontrado para "${buscaIns}"` : "Nenhum insumo cadastrado. Use '+ Novo Insumo' para cadastrar Sementes, Fertilizantes, Defensivos ou Corretivos."}
                    </div>
                  ) : (
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      ...
                      <tbody>
                        {insFiltr.map((ins, i) => {
                          ...
                          return (
                            <tr key={ins.id} style={{ borderBottom: i < insFiltr.length - 1 ? "0.5px solid var(--border-row)" : "none", background: abaixo ? "#FFFAF5" : "transparent" }}>
                              <td style={{ padding: "10px 14px" }}>
                ...
                {/* Modal Insumo */}
                {modalIns && (() => {
                  // Subgrupos sugeridos por categoria
                  const SUBGRUPOS_SUGERIDOS: Record<string, string[]> = {
                    defensivo:    ["Herbicida","Fungicida","Inseticida","Nematicida","Biológico","Adjuvante","Regulador de Crescimento"],
                    fertilizante: ["NPK Simples","NPK Complexo","Organomineral","Micronutriente","Inoculante","Líquido / Foliar","Orgânico"],
                    semente:      ["Convencional","Certificada","Híbrida","Tratada"],
                    corretivo:    ["Calcário Calcítico","Calcário Dolomítico","Gesso Agrícola","Silicato de Cálcio"],
                  };
                  const sugestoes = SUBGRUPOS_SUGERIDOS[fIns.categoria] ?? [];
                  const isComb = fIns.categoria === "combustivel";
                  return (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                      ...
                      <div>
                        ...
                        <select style={inp} value={fIns.categoria} onChange={e => {
                          ...
                          <option value="corretivo">Corretivos de Solo</option>
                          <option value="semente">Sementes</option>
                        </select>
                      </div>
                      {/* Subgrupo com sugestões contextuais */}
                      <div>
                        <label style={lbl}>Subgrupo</label>
                        <input
                          list={`subgrupo-sugestoes-${fIns.categoria}`}
                          style={inp}
                          placeholder={`Ex: ${sugestoes[0] ?? "Herbicida"}`}
                          value={fIns.subgrupo}
                          onChange={e => setFIns(p => ({ ...p, subgrupo: e.target.value }))}
                        />
                        {sugestoes.length > 0 && (
                          <datalist id={`subgrupo-sugestoes-${fIns.categoria}`}>
                      ...
                      <div>
                        ...
                        {editIns ? (
                          <div style={{ padding: "8px 10px", background: "var(--bg-card)", borderRadius: 7, border: "0.5px solid var(--border)", fontSize: 13, color: "var(--text-3)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const lista = await listarInsumosParaConta(contaId, fazIdEff ?? undefined);
                        setInsumos(lista);
          ...
          {/* ══ ITENS GERAIS ══ */}
          {aba === "itens" && (() => {
            // Categorias canônicas — "geral" é a nova padrão; legados continuam exibidos
            const CATS_IT: { key: Insumo["categoria"]; label: string; bg: string; cl: string }[] = [
              { key: "geral",       label: "Geral",         bg: "#F1F5F9", cl: "#475569" },
            ...
            // inclui legados no filtro — combustivel está na própria aba, não aparece aqui
            const CATS_IT_KEYS: Insumo["categoria"][] = ["geral","peca","material","uso_consumo","escritorio","outros"];
            ...
            const SUBCAT_GERAIS = ["Peças e Manutenção","Materiais de Obra","Uso e Consumo","Escritório e Papelaria","Ferramentas","EPI / Segurança","Outros"];
            ...
            return (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                  {itFiltr.length === 0 ? (
                    <div style={{ padding: 40, textAlign: "center", color: "#444", fontSize: 13 }}>
                      {buscaIt ? `Nenhum item encontrado para "${buscaIt}"` : "Nenhum item geral cadastrado. Use '+ Novo Item' para peças, materiais, escritório, ferramentas, etc."}
                    </div>
                  ) : (
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <TH cols={["Nome / Fabricante", "Subcategoria", "Unid.", "Estoque", "Mín.", "Valor Unit.", "Total", ""]} />
                      <tbody>
                        {itFiltr.map((ins, i) => {
                          const cat = catItMap[ins.categoria] ?? { label: ins.categoria, bg: "#F1F5F9", cl: "#475569" };
                          const abaixo = ins.estoque < ins.estoque_minimo;
                          const total  = ins.estoque * ins.valor_unitario;
                          return (
                            <tr key={ins.id} style={{ borderBottom: i < itFiltr.length - 1 ? "0.5px solid var(--border-row)" : "none", background: abaixo ? "#FFFAF5" : "transparent" }}>
                              ...
                              <td style={{ padding: "10px 14px", textAlign: "center" }}>
                                {ins.subgrupo
                                  ? <span style={{ fontSize: 11, background: "#F1F5F9", color: "#475569", padding: "2px 8px", borderRadius: 8 }}>{ins.subgrupo}</span>
                                  : <span style={{ fontSize: 10, background: cat.bg, color: cat.cl, padding: "2px 8px", borderRadius: 8, fontWeight: 600 }}>{cat.label}</span>}
                              </td>
                              <td style={{ padding: "10px 14px", textAlign: "center", fontSize: 12 }}>{ins.unidade}</td>
                              <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 600, color: abaixo ? "#E24B4A" : "var(--text-1)" }}>
                                {ins.estoque.toLocaleString("pt-BR", { maximumFractionDigits: 2 })}
                ...
                {modalIns && CATS_IT_KEYS.includes(fIns.categoria) && (
                  <Modal titulo={editIns ? `Editar: ${editIns.nome}` : "Novo Item Geral"} onClose={() => setModalIns(false)} width={680}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                      <div style={{ gridColumn: "1/-1" }}>
                        <label style={lbl}>Nome *</label>
                        <input style={inp} placeholder="Ex: Filtro de óleo, Graxeira, Resma de papel" value={fIns.nome} onChange={e => setFIns(p => ({ ...p, nome: e.target.value }))} />
                      </div>
                      <div>
                        <label style={lbl}>Subcategoria</label>
                        <input list="subcat-gerais-list" style={inp} placeholder="Ex: Peças e Manutenção" value={fIns.subgrupo} onChange={e => setFIns(p => ({ ...p, subgrupo: e.target.value }))} />
                        <datalist id="subcat-gerais-list">{SUBCAT_GERAIS.map(s => <option key={s} value={s} />)}</datalist>
                      </div>
                      <div>
                        <label style={lbl}>Unidade *</label>
                        <select style={inp} value={fIns.unidade} onChange={e => setFIns(p => ({ ...p, unidade: e.target.value as Insumo["unidade"] }))}>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <thead>
                        <tr style={{ background: "var(--bg-card)" }}>
                          <th style={{ padding: "8px 14px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Nome do Padrão</th>
                          <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Umidade</th>
                          <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Impureza</th>
                          <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Avariados</th>
                          {isSoja(comm) && <>
                            <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Ardidos</th>
                            <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Esverdeados</th>
                            <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Quebrados</th>
                            <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>PH mín.</th>
                          </>}
                          {isMilho(comm) && <>
                            <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Ardidos+Brot.</th>
                            <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Carunchados</th>
                            <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>PH mín.</th>
                          </>}
                          <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Kg/sc</th>
                          <th style={{ padding: "8px 10px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Status</th>
                          <th style={{ padding: "8px 10px", borderBottom: "0.5px solid var(--border-table)" }}></th>
                        </tr>
                      </thead>
                      <tbody>
                        {lista.map((p, pi) => (
                          <tr key={p.id} style={{ borderBottom: pi < lista.length - 1 ? "0.5px solid var(--border-row)" : "none" }}>
            ...
            <div>
              ...
              {subAbaUser === "grupos" && (
                <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                  ...
                  {grupos.map((g, gi) => (
                    <div key={g.id} style={{ padding: "12px 18px", borderBottom: gi < grupos.length - 1 ? "0.5px solid var(--border-row)" : "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
          ...
          {aba === "contas_bancarias" && (
            <div>
              ...
              {contas.length === 0 ? (
                ...
                <div style={{ border: "0.5px solid var(--border-table)", borderRadius: 12, overflow: "hidden" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead>
                      <tr style={{ background: "var(--bg-page)" }}>
                        {["Nome / Apelido", "Tipo", "Banco", "Conta", "Saldo Inicial", "Moeda", "Status", ""].map(h => (
                          <th key={h} style={{ padding: "10px 14px", textAlign: h === "Saldo Inicial" ? "right" : "left", fontWeight: 600, fontSize: 11, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
              ...
              {modalIU && (
                <Modal titulo={editIU ? "Editar Imóvel Urbano" : "Novo Imóvel Urbano"} subtitulo="Imóvel usado como garantia em operações financeiras" onClose={() => setModalIU(false)} width={760}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
                    ...
                    <div style={{ gridColumn: "1/-1" }}><hr style={{ border: "none", borderTop: "0.5px solid var(--border)", margin: "4px 0 6px" }} /></div>
                    <div>
            ...
            return (
              <div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, gap: 12, flexWrap: "wrap" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button style={{ ...btnV, background: "var(--bg-page)", color: "var(--text-2)", border: "0.5px solid var(--border-table)" }}
                    ...
                    {paFiltrados.map((pa, idx) => {
                      ...
                      return (
                        <div key={pa.id} style={{ borderBottom: idx < paFiltrados.length - 1 ? "0.5px solid #EEF1F7" : "none" }}>
                          {/* Linha principal */}
                          <div style={{ display: "flex", alignItems: "center", padding: "10px 14px", background: expandido ? "var(--bg-card)" : idx % 2 === 0 ? "#fff" : "#FAFBFD", gap: 12 }}>
                            <button
            ...
            return (
              <div>
                ...
                {unidades.length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    {porTipo.map(grupo => (
                      <div key={grupo.tipo} style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-table)", borderRadius: 10, overflow: "hidden" }}>
                        {/* Header do grupo */}
                        <div style={{ background: grupo.cor.bg, padding: "8px 14px", display: "flex", alignItems: "center", gap: 8, borderBottom: "0.5px solid var(--border-table)" }}>
                          ...
                          <thead>
                            <tr style={{ background: "#FAFBFC" }}>
                              {["Sigla", "Nome", "Conversão", "Status", ""].map((h, i) => (
                                <th key={i} style={{ padding: "7px 14px", textAlign: i >= 3 ? "center" : "left", fontSize: 10, fontWeight: 600, color: "var(--text-3)", borderBottom: "0.5px solid var(--bg-tag)" }}>{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {grupo.items.map(u => (
                              <tr key={u.id} style={{ borderBottom: "0.5px solid #F0F2F7", background: u.inativo ? "#FAFAFA" : "white" }}>
                                ...
                                <td style={{ padding: "8px 10px", textAlign: "right", whiteSpace: "nowrap" }}>
                                  <button onClick={() => abrirEditarUM(u)} style={{ fontSize: 11, padding: "3px 10px", border: "0.5px solid var(--border-table)", borderRadius: 6, background: "transparent", color: "var(--text-2)", cursor: "pointer", marginRight: 4 }}>
                                    Editar
                                  </button>
          ...
          {(() => {
            ...
            return (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 14 }}>
                ...
                {/* ── Titularidade ──────────────────────────────── */}
                <div style={{ gridColumn: "1 / -1", background: "var(--bg-card)", border: "0.5px solid var(--border)", borderRadius: 8, padding: "14px 16px" }}>
      ...
      {modalProd && (
        <Modal titulo={editProd ? "Editar Produtor" : "Novo Produtor"} onClose={() => setModalProd(false)} width={960}>
          ...
          {tabProd === "ies" && (<>
            <div style={{ fontSize: 12, color: "var(--text-2)", marginBottom: 16, background: "var(--bg-page)", border: "0.5px solid var(--border)", borderRadius: 8, padding: "10px 14px" }}>
          ...
          <Modal titulo={editFaz ? "Editar Fazenda" : "Nova Fazenda"} subtitulo="Uma fazenda pode pertencer a um Produtor (PF/parceria) ou a uma Empresa (PJ)" onClose={() => setModalFaz(false)} width={1100}>
            ...
            {tabFaz === "matriculas" && (
              <div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, marginBottom: 14 }}>
                  <thead><tr style={{ background: "var(--bg-page)" }}>{["N° Matrícula","Cartório","Área (ha)","Produtor","Garantia",""].map(h => <th key={h} style={{ padding: "7px 10px", textAlign: "left", fontSize: 11, color: "var(--text-2)", fontWeight: 600, borderBottom: "0.5px solid var(--border-table)" }}>{h}</th>)}</tr></thead>
                  <tbody>
            ...
            {tabFaz === "cars" && (
              <div>
                ...
                {fazCars.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "30px 20px", color: "var(--text-3)", fontSize: 13, border: "1.5px dashed var(--border-table)", borderRadius: 10 }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {fazArrendamentos.map((a, ai) => {
                    ...
                    return (
                      <div key={a._key} style={{ border: "0.5px solid var(--border-table)", borderRadius: 10, background: "#FAFBFC", overflow: "hidden" }}>
                        {/* Card header */}
                        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "var(--bg-page)", borderBottom: "0.5px solid var(--border-table)" }}>
                  ...
                  {fazArrendamentos.length === 0 && (
                    <div style={{ textAlign: "center", padding: 40, color: "var(--text-3)", fontSize: 13, border: "0.5px dashed var(--border-table)", borderRadius: 10 }}>
            ...
            {(() => {
              ...
              <div style={{ border: "0.5px solid var(--border-table)", borderRadius: 8, overflow: "hidden" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "var(--bg-page)" }}>
                      <th style={{ padding: "7px 12px", textAlign: "left",   fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Talhão</th>
                      <th style={{ padding: "7px 12px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Área total (ha)</th>
                      <th style={{ padding: "7px 12px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Disponível (ha)</th>
                      <th style={{ padding: "7px 12px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Área plantada (ha)</th>
                      <th style={{ padding: "7px 12px", textAlign: "center", fontSize: 11, fontWeight: 600, color: "var(--text-2)", borderBottom: "0.5px solid var(--border-table)" }}>Incluir</th>
                    </tr>
                  </thead>
                  <tbody>
                    {talhoesFazenda.map((t, ti, arr) => {
                      ...
                      return (
                        <tr key={t.id} style={{ borderBottom: ti < arr.length - 1 ? "0.5px solid var(--bg-tag)" : "none", background: excede ? "#FFF5F5" : marcado ? "#FAFEF8" : "transparent" }}>
      ...
      {modalBomba && (
        <Modal titulo={editBomba ? "Editar Bomba" : "Nova Bomba de Combustível"} onClose={() => setModalBomba(false)} width={720}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
            ...
            <div style={{ gridColumn: "1/-1", display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "var(--bg-page)", borderRadius: 8, border: "0.5px solid var(--border)" }}>
      ...
      {modalOpGer && (
        <Modal titulo={editOpGer ? "Editar Operação Gerencial" : "Nova Operação Gerencial"} onClose={() => setModalOpGer(false)} width={820}>
          ...
          {/* Sub-abas */}
          <div style={{ display: "flex", gap: 0, borderBottom: "0.5px solid var(--border-table)", marginBottom: 16 }}>
            {([
          ...
          <Modal titulo={editFunc ? `Funcionário — ${editFunc.nome}` : "Novo Funcionário"} onClose={() => setModalFunc(false)} width={920}>
            ...
            {abaFunc === "premiacoes" && (
              <div>
                    ...
                    <thead>
                      <tr style={{ background: "var(--bg-page)" }}>
                        {["Mês ref.", "Descrição", "Dt. Pagamento", "Valor", ""].map((h, i) => (
                          <th key={i} style={{ padding: "8px 12px", textAlign: i > 2 ? "right" : "left", fontWeight: 600, color: "var(--text-2)", fontSize: 11, borderBottom: "0.5px solid var(--border-row)" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                ...
                {premiacoes.length > 0 && (
                  <div style={{ marginTop: 12, padding: "10px 14px", background: "var(--bg-card)", borderRadius: 8, display: "flex", justifyContent: "flex-end", fontSize: 12 }}>
