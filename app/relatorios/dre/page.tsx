  ...
  async function calcularDre() {
    ...
    for (const cicloId of ciclosSel) {
      ...
      const [
        ...
        supabase.from("colheitas").select("peso_liquido_kg, sacas_liquidas, area_ha").eq("fazenda_id", cfid).eq("ciclo_id", cicloId).eq("status_campo", "aprovado"),
        supabase.from("adubacoes_base").select("custo_total").eq("fazenda_id", cfid).eq("ciclo_id", cicloId).eq("status_campo", "aprovado"),
        supabase.from("correcoes_solo").select("custo_total").eq("fazenda_id", cfid).eq("ciclo_id", cicloId).eq("status_campo", "aprovado"),
      ...
      for (const aux of auxCiclos) {
        ...
        const [
          ...
          supabase.from("plantios").select("custo_sementes").eq("fazenda_id", cfid).eq("ciclo_id", aux.id).eq("status_campo", "aprovado"),
          supabase.from("pulverizacoes").select("custo_total").eq("fazenda_id", cfid).eq("ciclo_id", aux.id).eq("status_campo", "aprovado"),
          Promise.resolve({ data: null }),
          supabase.from("adubacoes_base").select("custo_total").eq("fazenda_id", cfid).eq("ciclo_id", aux.id).eq("status_campo", "aprovado"),
          supabase.from("correcoes_solo").select("custo_total").eq("fazenda_id", cfid).eq("ciclo_id", aux.id).eq("status_campo", "aprovado"),
          Promise.resolve({ data: null }),
          Promise.resolve({ data: null }),
